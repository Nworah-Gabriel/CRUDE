...
const TokenVendor: NextPage = () => {
  ...
  const { data: YourTokenContractData } = useDeployedContractInfo("YourToken");
  const { data: yourTokenBalance } = useScaffoldReadContract({
    contractName: "YourToken",
    functionName: "balanceOf",
    args: [YourTokenContractData?.address],
  });
  ...
  return (
    <>
      <div className="flex items-center flex-col flex-grow pt-10">
        ...
        {!!yourTokenBalance && (
          <div className="flex flex-col items-center space-y-4 bg-base-100 shadow-lg shadow-secondary border-8 border-secondary rounded-xl p-6 mt-8 w-full max-w-lg">
            <div className="text-xl">Transfer tokens (input amount in ETH)</div>
            <div className="w-full flex flex-col space-y-2">
        ...
        {!!yourTokenBalance && (
          <div className="flex flex-col items-center space-y-4 bg-base-100 shadow-lg shadow-secondary border-8 border-secondary rounded-xl p-6 mt-8 w-full max-w-lg">
            ...
            <div>{tokensPerEth?.toString() || 0} tokens per ETH (input amount in ETH) </div>
